...
void TestSymbols() {
	Assert(IsPalindrom("ab ba"),"");
}

void SkipSymbol() {
	Assert(!IsPalindrom("ladam"),"");
}

void IgnoreSpace() {
	Assert(!IsPalindrom("ma dam"),"");
}

void NotMiddle() {
	Assert(!IsPalindrom("madtam"),"");
}
...
int main() {
  ...
  runner.RunTest(SkipSymbol,"SkipSymbol");
  runner.RunTest(IgnoreSpace,"IgnoreSpace");
  runner.RunTest(NotMiddle,"NotMiddle");
  runner.RunTest(TestCases,"TestCases");
  return 0;
}
